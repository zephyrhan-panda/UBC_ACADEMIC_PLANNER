...
* **Rank Specializations:** View a list of science majors ranked by their historical competitiveness and GPA requirements.
* **Prerequisite Check:** Input current courses and intended majors to identify missing required courses based on UBC’s official specialization requirements.
* **Probability Estimation:** Use a statistical model to calculate the likelihood of admission into specific majors based on the user's current GPA, accounting for historical "turbulence" and year-to-year fluctuations.
* **Strategic Recommendations:** Categorize majors into "Top Choice" (ambitious) and "Insurance Choice" (safe) based on the user's academic profile.
...
- As a user, I want to be able to **add a course** (including course code, credits, and grade) to my **academic record** so I can track my current standing.
